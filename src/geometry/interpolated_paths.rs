...
pub fn create_arc<F: Float>(x_0: F, y_0: F, r: F, theta_0: F, theta_1: F, n_points: usize) -> Path<F> {
    let mut xs = Vec::with_capacity(n_points);
    let mut ys = Vec::with_capacity(n_points);
...
impl<F> std::ops::Add for Path<F> where F: Float {
    type Output = Self;
